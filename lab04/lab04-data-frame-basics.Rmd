...
warriors_tbl2 <- data.frame(warriors_lst, stringsAsFactors = TRUE)
warriors_tbl2
warriors_tbl["Player"]
```
...
center_info <- centers[, c('player', 'salary')]
center_info
```
...
```{r}
max_height <- max(dat["height"])
max_height
```

- What's the minimum height value?
```{r}
min_height <- min(dat["height"])
min_height
```
...
```{r}
dat[which(dat$height == max_height),]
```

- Who is the shortest player?
```{r}
dat[which(dat$height==min(dat$height)),]
```
...
```{r}
dat[which(dat$age == max(dat$age)), ][, "player"]
```
...
```{r}
nrow(dat[dat$college == "University of California", ])
```
...
dat$ppm <- ppm
dat[dat$ppm == max(dat$ppm), ]$player
```
...
dat$tpm <- tpm
dat[dat$tpm == max(dat$tpm), ]$player
```
...
dat$twopm <- twopm
dat[dat$twopm == max(dat$twopm), ]$player
```

- Who is the player with the maximum rate of one-points (free-throws) per minute?
```{r}
opm <- dat$points1 / dat$minutes
dat$opm <- opm
dat[dat$opm == max(dat$opm), ]$player
```
...
aggregate(. ~ team, data = dat[, c('team', 'height', 'weight', 'age')], FUN = mean)
```
...
```{r}
aggregate(dat$salary, by=list(team = dat$team), FUN = function(x) c(min = min(x), med = median(x), avg = mean(x), max = max(x)))
```
