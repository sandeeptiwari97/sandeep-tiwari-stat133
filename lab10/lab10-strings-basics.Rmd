...
paste(first_five, "= ", nchar(first_five))
```
...
```{r}
states_list <- list()
freqs = strtoi(names(char_freqs))
for(i in 1:length(freqs)) {
  if (freqs[i] %% 2 == 0) {
    states_list[[i]] = toupper(states[num_chars == freqs[i]])
  } else {
    states_list[[i]] = tolower(states[num_chars == freqs[i]])
  }
  names(states_list)[i] = paste(freqs[i], "chars", sep="-")
}
print(states_list)
```
-----
...
cat("---", file = outfile)
cat("\ntitle: Lab 10: Basics of strings manipulations", file = outfile, append = TRUE)
cat("\nauthor: Sandeep Tiwari", file = outfile, append = TRUE)
cat("\ndate: 11/3/17", file = outfile, append = TRUE)
...
colplot <- function(col) {
  if (!is_color(col)) {
    return(paste("invalid color", col))
  } else {
    r <- runif(100)
    d <- data.frame(
      x = 1:100,
...
for(i in 1:length(vowels)) {
  print(paste(vowels[i], sum(letrs == vowels[i]), sep = ": "))
}
```
...
count_letters <- function(letrs) {
  ...
  for(i in 1:length(letrs)) {
    if (letrs[i]%in%vowels) {
      vows = vows + 1
    } else {
      cons = cons + 1
    }
  }
  print(paste("letters:", lets))
  print(paste("vowels:", vows))
  print(paste("consonants:", cons))
}
count_letters(letrs)
```